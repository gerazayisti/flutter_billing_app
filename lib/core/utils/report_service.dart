...
class ReportService {
  static Future<void> generateDailyReport(DateTime date) async {
    ...
    final pdf = await _buildReportPdf(
      title: 'Rapport Journalier - ${DateFormat('dd/MM/yyyy').format(date)}',
      orders: orders,
      showDetails: true,
    );
  ...
  static Future<void> generateWeeklyReport(DateTime date) async {
    ...
    final pdf = await _buildReportPdf(
      title: 'Rapport Hebdomadaire - Semaine du ${DateFormat('dd/MM').format(startOfWeek)}',
      orders: orders,
      showDetails: true,
      recaps: _getDailyRecaps(orders),
      recapTitle: 'Récapitulatif Journalier',
    );
    await Printing.layoutPdf(onLayout: (format) async => pdf.save());
  }

  static Future<void> generateMonthlyReport(DateTime date) async {
    ...
    final pdf = await _buildReportPdf(
      title: 'Rapport Mensuel - ${DateFormat('MMMM yyyy').format(date)}',
      orders: orders,
      showDetails: true,
      recaps: _getWeeklyRecaps(orders),
      recapTitle: 'Récapitulatif Hebdomadaire',
    );
    await Printing.layoutPdf(onLayout: (format) async => pdf.save());
  }
  ...
  static Map<String, double> _getWeeklyRecaps(List<OrderModel> orders) {
    final Map<String, double> recaps = {};
    for (var o in orders) {
      final weekNum = ((o.date.day - 1) / 7).floor() + 1;
      final key = 'Semaine $weekNum';
      recaps[key] = (recaps[key] ?? 0) + o.totalAmount;
    }
    return recaps;
  }

  static Map<String, double> _getCategoryBreakdown(List<OrderModel> orders) {
    ...
    for (var order in orders) {
      for (var item in order.items) {
        final product = products.firstWhere((p) => p.id == item.productId, 
          orElse: () => ProductModel(id: '', name: item.productName, barcode: '', price: item.price, stock: 0, category: 'Inconnue', minStockAlert: 0, variants: []));
  ...
  static Future<pw.Document> _buildReportPdf({
    ...
    final categoryBreakdown = _getCategoryBreakdown(orders);
      ...
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.Row(
            ...
            children: [
              pw.Text('CHIFFRE D\'AFFAIRES TOTAL :', style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 18)),
          ...
          // Category Breakdown
          pw.Header(level: 1, text: 'Détail par Catégorie'),
          pw.TableHelper.fromTextArray(
            headers: ['Catégorie', 'CA (XAF)', '%'],
          ...
          if (showDetails) ...[
            pw.Header(level: 1, text: 'Liste des Transactions'),
            pw.TableHelper.fromTextArray(
              headers: ['ID', 'Date', 'Mode', 'Montant (XAF)'],
              data: orders.map((o) => [
                o.id.substring(0, 8),
